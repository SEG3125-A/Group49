...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        ...
        var choose = readData("choose");
        var speed = readData("speed");
        res.render('showResults', {results: [role, rate, appealing, choose,speed]});
        console.log([role, rate, appealing, choose,speed]);
    });
    ...
    // when a user goes to localhost:3000/lab1Survey
    // serve a static html (the survey itself to fill in)
    app.get('/lab1Survey', function(req, res){
        res.sendFile(__dirname+'/views/lab1Survey.html');
    });

    // when a user types SUBMIT in localhost:3000/lab1Survey 
    // the action.js code will POST, and what is sent in the POST
    // will be recuperated here, parsed and used to update the data files
    app.post('/lab1Survey', urlencodedParser, function(req, res){
        console.log(req.body);
        var json = req.body;
        for (var key in json){
            console.log(key + ": " + json[key]);
            // in the case of checkboxes, the user might check more than one
            if ((key === "choose") && (json[key].length === 2)){
                for (var item in json[key]){
                    combineCounts(key, json[key][item]);
                }
            }
